...
parent_bitbang_shift_register::parent_bitbang_shift_register(){
	//set outputs
	//SER 0
	//NOE 1
	//RCLK 2
...
void parent_bitbang_shift_register::parent_shiftbyte(byte setByte){
...
void parent_bitbang_shift_register::parent_shiftbyte_lsb(byte setByte){
	//clear the shift register, clear clocks, clear serial input
	PORTD &= ~(1 << SRCLK | 1 << RCLK | 1 << SER);
	//disable output
	PORTD |=  (1 << NOE | 1 << SRCLR);
...
//clears the board
void parent_bitbang_shift_register::parent_clearboard(){
PORTD &= ~(1<<SRCLR);
}
